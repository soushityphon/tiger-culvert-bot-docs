...
The profile can include current IGN, weekly job, Tiger Rank, same-job rank, latest and personal-best scores, change from the previous result, distance from the personal best, recent form, participation, and separate On Vacation / Upcoming Vacation date ranges with approximate duration. Job Rank remains visible even when the member is the only recorded player for that job. When Nexon returns a validated GMS character record, the profile also shows the Nexon character image as the top-right thumbnail. Tiger Rank and same-job rank use the complete newest Weekly Data roster, including members with a Culvert score of 0; equal scores share the same rank. The self-facing view does not expose the internal Player ID; admin `/whois` retains it.
